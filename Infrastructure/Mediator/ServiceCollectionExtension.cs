...
namespace Infrastructure.Mediator
{
    public static class ServiceCollectionExtension
    {
        public static IServiceCollection AddInProcessMessageBus(this IServiceCollection services)
        {
            ...
            ConnectImplementationsToTypesClosing(typeof(ICommandHandler<>), services, assemblies, false);
            ConnectImplementationsToTypesClosing(typeof(IQueryHandler<,>), services, assemblies, false);

            var multipleOpenInterfaces = new[]
            {
                typeof(ICommandHandler<>),
                typeof(IQueryHandler<,>)
            };

            foreach (var handler in multipleOpenInterfaces)
            {
                var concretions = assemblies.SelectMany(a => a.DefinedTypes)
                                            .Where(type => type.FindInterfacesThatClose(handler).Any())
